...
void Input::Update()
{
	...
	for(int i=0; i<4; i++)
	{
		if(m_lastgamepad[i]!=m_gamepad[i])
		{
			m_samestate[i]=false;
		}
	}
...
void Input::DetectChangeState(const uint8_t gamepad)
{
	if(gamepad<1 || gamepad>4)
	{
		return;
	}
	m_samestate[gamepad-1]=true;
}

bool Input::SameState(const uint8_t gamepad) const
{
	if(gamepad<1 || gamepad>4)
	{
		return false;
	}
	return m_samestate[gamepad-1];
}
